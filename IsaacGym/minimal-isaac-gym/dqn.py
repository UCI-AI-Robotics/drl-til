...
class DQN:
    ...
    def update(self):
        ...
        # act: tensor([0., 1., 0.,  ..., 0., 0., 1.], device='cuda:0')
        # act.long: tensor([0, 1, 0,  ..., 0, 0, 1], device='cuda:0')

        # q_table: tensor([[ 0.4579,  0.4390],
        # [ 0.3551,  0.3232],
        # [-0.1965, -0.0610],
        # ...,
        # [-0.3546,  0.1595],
        # [ 0.5732,  0.5636],
        # [ 0.3713,  0.3390]], device='cuda:0', grad_fn=<AddmmBackward0>)
        # q_val: tensor([ 0.4579,  0.3232, -0.0610,  ..., -0.3546,  0.5732,  0.3713],
        # q_table: torch.Size([65536, 2]) / q_val: torch.Size([65536])
    ...
    def act(self, obs, epsilon=0.0):
        ...
        with torch.no_grad():
            ...
            # true_act = torch.cat([
            #     (q_table[b] == q_table[b].max()).nonzero(as_tuple=False)[0]
            #     for b in range(self.args.num_envs)
            # ])

            # max_mask: tensor([[1]], device='cuda:0')
            # max_mask2: tensor([False,  True], device='cuda:0')
            # max_mask: tensor([[0]], device='cuda:0')
            # max_mask2: tensor([ True, False], device='cuda:0')
            # max_mask: tensor([[0]], device='cuda:0')
            # max_mask2: tensor([ True, False], device='cuda:0')
            # max_mask: tensor([[1]], device='cuda:0')
            # max_mask2: tensor([False,  True], device='cuda:0')
            ...
            true_acts = []
            for b in range(self.args.num_envs):
                # Create a mask where the max value condition is true
                max_mask = (q_table[b] == q_table[b].max()).nonzero(as_tuple=False)[0]
                max_mask2 = (q_table[b] == q_table[b].max())
                true_acts.append(max_mask)
            true_act = torch.cat(true_acts)
        ...
        return 2 * (act - 0.5)  # maps from 0 - 1 to -1 to 1
